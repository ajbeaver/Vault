...
class VaultTUI(App[None]):
    ...
    BINDINGS = [
        ...
        ("6", "show_send", "Send"),
        ("7", "show_smart_accounts", "Smart"),
        ("8", "show_policy", "Policy"),
        ("9", "show_journal", "Journal"),
        ("0", "show_ops", "Ops"),
        ("d", "switch_to_dev", "Dev"),
    ...
    def __init__(self, home: str | None = None, profile: str | None = None, allow_prod: bool = False) -> None:
        ...
        self.smart_rows: list[dict[str, Any]] = []
        ...
        self.selected_smart_name: str | None = None
        self.selected_journal_hash: str | None = None
        self.last_user_operation: dict[str, Any] | None = None
        ...
        with Horizontal(id="top"):
            ...
            with Vertical(id="main"):
                ...
                with TabbedContent(id="tabs"):
                        ...
                        with Horizontal(classes="master-detail"):
                            ...
                            with Vertical(classes="detail-column"):
                                ...
                                with Vertical(classes="form-block"):
                                    ...
                                    yield Input(value="local", placeholder="Name", id="network_anvil_name")
                                    yield Input(value="http://127.0.0.1:8545", placeholder="RPC URL", id="network_anvil_rpc_url")
                                    yield Input(value="31337", placeholder="Chain ID", id="network_anvil_chain_id")
                                    yield Input(value="ETH", placeholder="Symbol", id="network_anvil_symbol")
                                    yield Button("Add Anvil", id="network_anvil_button")
                                with Vertical(classes="form-block"):
                                    yield Label("Add Alchemy network", classes="section-title")
                                    yield Input(placeholder="Preset, e.g. eth-sepolia", id="network_alchemy_preset")
                                    yield Input(placeholder="Local name", id="network_alchemy_name")
                                    yield Input(value="ALCHEMY_API_KEY", placeholder="API key env", id="network_alchemy_env")
                    ...
                    with TabPane("Smart", id="tab_smart"):
                        with Horizontal(classes="master-detail"):
                            with Vertical(classes="list-column"):
                            ...
                            with Vertical(classes="detail-column"):
                                ...
                                with Horizontal(classes="form-actions"):
                                    yield Button("Set Default", id="smart_set_default_button", variant="primary")
                                    yield Button("Use Network", id="smart_use_network_button")
                                    yield Button("Remove Selected", id="smart_remove_selected_button", variant="error")
                                with Vertical(classes="form-block"):
                                    yield Label("Register Safe", classes="section-title")
                                    yield Input(placeholder="Name", id="safe_name")
                                    yield Input(placeholder="Safe address", id="safe_address")
                                    yield Input(placeholder="Network", id="safe_network")
                                    yield Input(placeholder="Safe service URL (optional)", id="safe_service_url")
                                    yield Input(placeholder="Entrypoint (optional)", id="safe_entrypoint")
                                    yield Button("Register Safe", id="safe_register_button")
                                with Vertical(classes="form-block"):
                                    yield Label("Register ERC-4337", classes="section-title")
                                    yield Input(placeholder="Name", id="aa_name")
                                    yield Input(placeholder="Sender address", id="aa_sender")
                                    yield Input(placeholder="Network", id="aa_network")
                                    yield Input(placeholder="Owner account", id="aa_owner_account")
                                    yield Input(placeholder="Entrypoint", id="aa_entrypoint")
                                    yield Input(value="0.6", placeholder="Version", id="aa_version")
                                    yield Input(placeholder="Bundler URL (optional)", id="aa_bundler_url")
                                    yield Input(placeholder="Paymaster URL (optional)", id="aa_paymaster_url")
                                    yield Button("Register ERC-4337", id="aa_register_button")
                        ...
                        with Horizontal(classes="master-detail"):
                            ...
                            with Vertical(classes="detail-column"):
                                ...
                                with Horizontal(classes="form-actions"):
                                    ...
                                    yield Button("Load Selected Hash", id="journal_load_hash_button")
                                with Vertical(classes="form-block"):
                    ...
                    with TabPane("Ops", id="tab_ops"):
                        with Horizontal(classes="send-layout"):
                            with Vertical(classes="send-form-column"):
                                ...
                                yield Input(password=True, placeholder="Passphrase", id="ops_passphrase")
                                with Vertical(classes="form-block"):
                                    yield Label("Safe", classes="section-title")
                                    yield Input(placeholder="To address", id="ops_safe_to")
                                    yield Input(value="0", placeholder="Value wei", id="ops_safe_value")
                                    yield Input(value="0x", placeholder="Data", id="ops_safe_data")
                                    yield Input(placeholder="Nonce (optional)", id="ops_safe_nonce")
                                    yield Input(placeholder="Origin (optional)", id="ops_safe_origin")
                                    yield Input(placeholder="Safe tx hash", id="ops_safe_tx_hash")
                                    with Horizontal(classes="form-actions"):
                                        yield Button("Pending", id="ops_safe_pending_button")
                                        yield Button("Load Tx", id="ops_safe_tx_button")
                                    with Horizontal(classes="form-actions"):
                                        yield Button("Propose", id="ops_safe_propose_button", variant="primary")
                                        yield Button("Confirm", id="ops_safe_confirm_button")
                                        yield Button("Execute", id="ops_safe_execute_button", variant="error")
                                with Vertical(classes="form-block"):
                                    yield Label("ERC-4337", classes="section-title")
                                    yield Input(placeholder="To address", id="ops_aa_to")
                                    yield Input(value="0", placeholder="Value wei", id="ops_aa_value")
                                    yield Input(value="0x", placeholder="Data", id="ops_aa_data")
                                    yield Input(placeholder="Nonce (optional)", id="ops_aa_nonce")
                                    yield Input(placeholder="UserOp hash", id="ops_aa_userop_hash")
                                    with Horizontal(classes="form-actions"):
                                        yield Button("Prepare", id="ops_aa_prepare_button", variant="primary")
                                        yield Button("Sign", id="ops_aa_sign_button")
                                        yield Button("Simulate", id="ops_aa_simulate_button")
                                    with Horizontal(classes="form-actions"):
    ...
    def on_mount(self) -> None:
        ...
        self.query_one("#ops_result_view", Static).update(self.render_ops_placeholder())
    ...
    def action_show_smart_accounts(self) -> None:
        self.switch_tab("tab_smart", "smart_list")
    ...
    def action_show_ops(self) -> None:
        self.switch_tab("tab_ops", "ops_smart_name")
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        handlers = {
            ...
            "smart_set_default_button": self.set_selected_smart_default,
            "smart_use_network_button": self.use_selected_smart_network,
            "smart_remove_selected_button": self.remove_selected_smart,
            "safe_register_button": self.register_safe,
            "aa_register_button": self.register_erc4337,
            ...
            "journal_load_hash_button": self.load_selected_journal_hash,
            "journal_receipt_button": self.fetch_journal_receipt,
            "ops_safe_pending_button": self.load_safe_pending,
            "ops_safe_tx_button": self.load_safe_transaction,
            "ops_safe_propose_button": self.propose_safe_transaction,
            "ops_safe_confirm_button": self.confirm_safe_transaction,
            "ops_safe_execute_button": self.execute_safe_transaction,
            "ops_aa_prepare_button": self.prepare_user_operation,
            "ops_aa_sign_button": self.sign_user_operation,
            "ops_aa_simulate_button": self.simulate_user_operation,
            "ops_aa_submit_button": self.submit_user_operation,
            "ops_aa_status_button": self.load_user_operation_status,
        }
    ...
    def on_list_view_highlighted(self, event: ListView.Highlighted) -> None:
        item = event.item
        if not isinstance(item, EntityListItem):
            return
    ...
    def refresh_all_views(self, refresh_balance: bool = False, log_message: str | None = None) -> None:
        ...
        smart_payload = self.service.list_smart_accounts()
        ...
        self.smart_rows = smart_payload["accounts"]
        self.journal_rows = journal_payload["entries"]

        self.refresh_sidebar(refresh_balance=refresh_balance)
        self.refresh_master_detail_lists(accounts_payload, networks_payload, profiles_payload, smart_payload, journal_payload)
        ...
        self.query_one("#smart_detail", Static).update(self.render_smart_detail(self.find_row(self.smart_rows, self.selected_smart_name)))
        self.query_one("#journal_detail", Static).update(self.render_journal_detail(self.find_journal_row(self.selected_journal_hash)))
    ...
    def refresh_context_bar(self) -> None:
        context = self.service.context_summary()
        safety = self.service.safety_status()
        self.query_one("#context_bar", Static).update(self.render_context_bar(context, safety))
    ...
    def refresh_master_detail_lists(
        ...
        self.rebuild_list(
            "profiles_list",
            self.profile_rows,
            self.make_profile_item,
            self.selected_profile_name or profiles_payload["active_profile"],
        )
        self.rebuild_list(
            "accounts_list",
            self.account_rows,
            self.make_account_item,
            self.selected_account_name or accounts_payload.get("default_account"),
        )
        self.rebuild_list(
            "networks_list",
            self.network_rows,
            self.make_network_item,
            self.selected_network_name or networks_payload.get("default_network"),
        )
        self.rebuild_list(
            "book_list",
            self.book_rows,
        ...
        self.rebuild_list(
            "smart_list",
            self.smart_rows,
            self.make_smart_item,
            self.selected_smart_name or smart_payload.get("default_smart_account"),
        )
        self.rebuild_list(
            "journal_list",
            self.journal_rows,
            self.make_journal_item,
            self.selected_journal_hash or first_journal_hash(journal_payload["entries"]),
        )
    ...
    def switch_profile(self, name: str) -> None:
        ...
        self.selected_smart_name = None
        self.selected_journal_hash = None
    ...
    def create_account(self) -> None:
        name = self.value("account_create_name")
        passphrase = self.value("account_create_passphrase")
        confirm = self.value("account_create_passphrase_confirm")
        if passphrase != confirm:
            raise VaultError("Passphrases do not match.")
        payload = self.service.create_account(name=name, passphrase=passphrase, set_default=True)
    ...
    def import_account(self) -> None:
        ...
        private_key = self.value("account_import_private_key")
        passphrase = self.value("account_import_passphrase")
        confirm = self.value("account_import_passphrase_confirm")
        if passphrase != confirm:
            raise VaultError("Passphrases do not match.")
        payload = self.service.import_account(name=name, private_key_hex=private_key, passphrase=passphrase, set_default=True)
        self.selected_account_name = payload["name"]
        self.last_balance_snapshot = None
        self.refresh_all_views(refresh_balance=True, log_message=payload["summary"])
    ...
    def add_book_entry(self) -> None:
        payload = self.service.add_address_book_entry(
            name=self.value("book_name"),
            address=self.value("book_address"),
    ...
    def set_selected_smart_default(self) -> None:
        if not self.selected_smart_name:
            raise VaultError("No smart account selected.")
        payload = self.service.use_smart_account(self.selected_smart_name)
        self.refresh_all_views(log_message=payload["summary"])

    def use_selected_smart_network(self) -> None:
        row = self.find_row(self.smart_rows, self.selected_smart_name)
        if not row:
            raise VaultError("No smart account selected.")
        self.set_value("balance_network", row["network"])
        self.set_value("send_network", row["network"])
        self.set_value("ops_smart_name", row["name"])
        self.write_log(f"Loaded smart account {row['name']} network into forms.")

    def remove_selected_smart(self) -> None:
        if not self.selected_smart_name:
            raise VaultError("No smart account selected.")
        payload = self.service.remove_smart_account(self.selected_smart_name)
        self.selected_smart_name = None
        self.refresh_all_views(log_message=payload["summary"])

    def register_safe(self) -> None:
        payload = self.service.register_safe_account(
            name=self.value("safe_name"),
            address=self.value("safe_address"),
            network_name=self.value("safe_network"),
            service_url=self.value("safe_service_url") or None,
            entrypoint=self.value("safe_entrypoint") or None,
            set_default=not self.smart_rows,
        )
        self.selected_smart_name = payload["name"]
        self.refresh_all_views(log_message=payload["summary"])

    def register_erc4337(self) -> None:
        payload = self.service.register_erc4337_account(
            name=self.value("aa_name"),
            sender=self.value("aa_sender"),
            network_name=self.value("aa_network"),
            owner_account=self.value("aa_owner_account"),
            entrypoint=self.value("aa_entrypoint"),
            version=self.value("aa_version") or "0.6",
            bundler_url=self.value("aa_bundler_url") or None,
            paymaster_url=self.value("aa_paymaster_url") or None,
            set_default=not self.smart_rows,
        )
        self.selected_smart_name = payload["name"]
        self.refresh_all_views(log_message=payload["summary"])
    ...
    def use_selected_account_in_forms(self) -> None:
        ...
        self.write_log(f"Loaded account {self.selected_account_name} into balance and send forms.")
    ...
    def use_selected_network_in_forms(self) -> None:
        ...
        self.write_log(f"Loaded network {self.selected_network_name} into balance and send forms.")
    ...
    def set_policy_rule(self) -> None:
        payload = self.service.set_policy_rule(
            rule=self.value("policy_rule"),
            value=self.value("policy_value"),
            account_name=self.value("policy_scope_account") or None,
        )
        self.refresh_policy_view(log_message=False)
        self.query_one("#policy_explain_view", Static).update(self.render_policy_explain_placeholder())
        self.write_log(payload["summary"])
    ...
    def unset_policy_rule(self) -> None:
        payload = self.service.unset_policy_rule(
            rule=self.value("policy_rule"),
            account_name=self.value("policy_scope_account") or None,
        )
        self.refresh_policy_view(log_message=False)
    ...
    def load_selected_journal_hash(self) -> None:
        row = self.find_journal_row(self.selected_journal_hash)
        if not row:
            raise VaultError("No journal entry selected.")
        self.set_value("journal_tx_hash", row["tx_hash"])
        if row.get("network"):
            self.set_value("journal_network", row["network"])
        self.write_log(f"Loaded journal hash {row['tx_hash']}.")

    def fetch_journal_receipt(self) -> None:
        tx_hash = self.value("journal_tx_hash") or self.selected_journal_hash
        if not tx_hash:
            raise VaultError("Provide a transaction hash or select a journal entry first.")
    ...
    def load_safe_pending(self) -> None:
        payload = self.service.list_safe_pending_transactions(self.ops_smart_name())
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def load_safe_transaction(self) -> None:
        safe_tx_hash = self.value("ops_safe_tx_hash")
        payload = self.service.show_safe_pending_transaction(safe_tx_hash, self.ops_smart_name())
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def propose_safe_transaction(self) -> None:
        payload = self.service.propose_safe_transaction(
            name=self.ops_smart_name(),
            signer_account=self.value("ops_signer_account"),
            passphrase=self.value("ops_passphrase"),
            to=self.value("ops_safe_to"),
            value=self.value("ops_safe_value") or "0",
            data=self.value("ops_safe_data") or "0x",
            nonce=int_or_none(self.value("ops_safe_nonce")),
            origin=self.value("ops_safe_origin") or None,
        )
        if payload.get("safe_tx_hash"):
            self.set_value("ops_safe_tx_hash", payload["safe_tx_hash"])
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def confirm_safe_transaction(self) -> None:
        payload = self.service.confirm_safe_transaction(
            name=self.ops_smart_name(),
            signer_account=self.value("ops_signer_account"),
            passphrase=self.value("ops_passphrase"),
            safe_tx_hash=self.value("ops_safe_tx_hash"),
    ...
    def execute_safe_transaction(self) -> None:
        payload = self.service.execute_safe_transaction(
            name=self.ops_smart_name(),
            signer_account=self.value("ops_signer_account"),
            passphrase=self.value("ops_passphrase"),
            safe_tx_hash=self.value("ops_safe_tx_hash"),
    ...
    def prepare_user_operation(self) -> None:
        payload = self.service.prepare_user_operation(
            name=self.ops_smart_name(),
            to=self.value("ops_aa_to"),
            value=self.value("ops_aa_value") or "0",
            data=self.value("ops_aa_data") or "0x",
            nonce=self.value("ops_aa_nonce") or None,
        )
        self.last_user_operation = payload["user_operation"]
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def sign_user_operation(self) -> None:
        if not self.last_user_operation:
            raise VaultError("No prepared user operation loaded. Prepare one first.")
        payload = self.service.sign_user_operation(
            name=self.ops_smart_name(),
            passphrase=self.value("ops_passphrase"),
            user_operation=self.last_user_operation,
        )
        self.last_user_operation = payload["user_operation"]
        if payload.get("user_operation_hash"):
            self.set_value("ops_aa_userop_hash", payload["user_operation_hash"])
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def simulate_user_operation(self) -> None:
        if not self.last_user_operation:
            raise VaultError("No prepared user operation loaded. Prepare one first.")
        payload = self.service.simulate_user_operation(self.ops_smart_name(), self.last_user_operation)
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def submit_user_operation(self) -> None:
        if not self.last_user_operation:
            raise VaultError("No prepared user operation loaded. Prepare one first.")
        payload = self.service.submit_user_operation(self.ops_smart_name(), self.last_user_operation)
        if payload.get("user_operation_hash"):
            self.set_value("ops_aa_userop_hash", payload["user_operation_hash"])
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def load_user_operation_status(self) -> None:
        user_operation_hash = self.value("ops_aa_userop_hash")
        payload = self.service.user_operation_status(self.ops_smart_name(), user_operation_hash)
        self.query_one("#ops_result_view", Static).update(self.render_data_panel(payload))
        self.write_log(payload["summary"])

    def fetch_balance(self) -> None:
        snapshot = self.service.balance_snapshot(
        ...
        self.last_balance_snapshot = snapshot
        self.refresh_balance_views()
        self.write_log(snapshot["summary"])
    ...
    def broadcast_send(self) -> None:
        ...
        payload = self.service.execute_send(
            passphrase=self.value("send_passphrase"),
        ...
        self.write_log(f"Broadcasted transaction {payload['transaction_hash']}")

    def on_input_changed(self, event: Input.Changed) -> None:
        widget_id = event.input.id or ""
        if widget_id in SEND_INPUT_IDS:
    ...
    def ops_smart_name(self) -> str | None:
        return self.value("ops_smart_name") or self.selected_smart_name or None
    ...
    def cycle_theme(self, step: int) -> None:
        ...
        self.apply_theme()
        self.refresh_all_views(refresh_balance=False, log_message=f"Theme set to {payload['name']}.")
    ...
    def find_journal_row(self, tx_hash: str | None) -> dict[str, Any] | None:
        if not tx_hash:
    ...
    def update_detail_for_list(self, list_id: str, entity_name: str) -> None:
        if list_id == "profiles_list":
            ...
            self.query_one("#profiles_detail", Static).update(
                self.render_profile_detail(self.find_row(self.profile_rows, entity_name))
            )
        elif list_id == "accounts_list":
            self.selected_account_name = entity_name
            self.query_one("#accounts_detail", Static).update(
                self.render_account_detail(self.find_row(self.account_rows, entity_name))
            )
        elif list_id == "networks_list":
            self.selected_network_name = entity_name
            self.query_one("#networks_detail", Static).update(
                self.render_network_detail(self.find_row(self.network_rows, entity_name))
            )
        elif list_id == "book_list":
            self.selected_book_name = entity_name
            self.query_one("#book_detail", Static).update(
                self.render_book_detail(self.find_row(self.book_rows, entity_name))
            )
        elif list_id == "smart_list":
            self.selected_smart_name = entity_name
            self.query_one("#smart_detail", Static).update(
                self.render_smart_detail(self.find_row(self.smart_rows, entity_name))
            )
        elif list_id == "journal_list":
            self.selected_journal_hash = entity_name
            self.query_one("#journal_detail", Static).update(
                self.render_journal_detail(self.find_journal_row(entity_name))
            )
    ...
    def make_smart_item(self, row: dict[str, Any]) -> EntityListItem:
        marker = "*" if row["is_default"] else " "
        kind = row.get("type", "smart")
        return EntityListItem(row["name"], f"{marker} {row['name']}", f"{kind} · {row['network']}")

    def make_journal_item(self, row: dict[str, Any]) -> EntityListItem:
        label = row.get("action", "event")
        status = row.get("status", "-")
        return EntityListItem(row["tx_hash"], shorten_hash(row["tx_hash"]), f"{label} · {status}")
    ...
    def render_profile_card(self, context: dict[str, Any]) -> str:
        ...
        return (
            ...
            f"Smart: {context['smart_account_count']}\n"
            f"Theme: {context['theme']}"
        )

    def render_context_bar(self, context: dict[str, Any], safety: dict[str, Any]) -> str:
        account = context["default_account"]
        network = context["default_network"]
        smart = context.get("default_smart_account")
        ...
        smart_text = smart["name"] if smart else "No default smart account"
        if smart:
            smart_text += f" · {smart.get('type', 'smart')}"
        issue_count = 0
        findings = safety.get("findings") or []
        if findings and findings[0] != "No immediate safety issues detected.":
            issue_count = len(findings)
        mode = "Protected send rules active" if context["is_protected_profile"] else "Workspace mode"
        return (
            f"[b]Active Context[/b]\n"
            f"Profile: {context['profile']} · {mode}\n"
            f"Theme: {context['theme']}\n"
            f"Account: {account_text}\n"
            f"Network: {network_text}\n"
            f"Smart: {smart_text}\n"
            f"Safety Findings: {issue_count}"
        )
    ...
    def render_account_card(self, context: dict[str, Any]) -> str:
        ...
        return (
            "[b]Default Account[/b]\n"
            f"{account['name']}\n"
            f"{shorten_address(account['address'])}"
        )
    ...
    def render_network_card(self, context: dict[str, Any]) -> str:
        ...
        return (
            "[b]Default Network[/b]\n"
            f"{network['name']}\n"
            f"{network.get('provider', 'custom')} · {network['chain_id']}"
        )
    ...
    def render_balance_card(self, snapshot: dict[str, Any] | None) -> str:
        ...
        return (
            "[b]Balance Snapshot[/b]\n"
            f"{snapshot['balance']} {snapshot['symbol']}\n"
            f"{snapshot['account_name']} @ {snapshot['network']}"
        )
    ...
    def render_account_detail(self, row: dict[str, Any] | None) -> str:
        ...
        default = "[green]default[/green]" if row["is_default"] else "secondary"
        return (
            f"[b]{row['name']}[/b]\n"
            f"Role: {default}\n"
    ...
    def render_smart_detail(self, row: dict[str, Any] | None) -> str:
        if not row:
            return "No smart accounts available."
        details = [
            f"[b]{row['name']}[/b]",
            f"Default: {yes_no(row['is_default'])}",
            f"Type: {row.get('type', 'smart')}",
            f"Address: {row['address']}",
            f"Network: {row['network']}",
        ]
        if row.get("type") == "safe":
            details.extend(
                [
                    f"Threshold: {row.get('threshold')}",
                    f"Owners: {', '.join(row.get('owners') or []) or '-'}",
                    f"Service URL: {row.get('service_url') or '-'}",
                ]
            )
        if row.get("type") == "erc4337":
            details.extend(
                [
                    f"Owner Account: {row.get('owner_account')}",
                    f"Entrypoint: {row.get('entrypoint')}",
                    f"Version: {row.get('version')}",
                    f"Bundler: {row.get('bundler_url') or '-'}",
                ]
            )
        return "\n".join(details)
    ...
    def render_journal_detail(self, row: dict[str, Any] | None) -> str:
        ...
        return (
            ...
            f"Smart Account: {row.get('smart_account') or '-'}\n"
            f"Account: {row.get('account_name') or '-'}\n"
            f"Created: {row.get('created_at') or '-'}\n"
            f"To: {row.get('to_address') or '-'}\n"
    ...
    def render_policy_explain_placeholder(self) -> str:
        return "[b]No evaluation loaded[/b]\nExplain a candidate action to see allow/deny results."

    def render_policy_view(self, payload: dict[str, Any]) -> str:
        return (
            f"[b]{payload['scope']} policy[/b]\n"
            f"{pretty_json(payload['policy'])}"
        )
    ...
    def render_receipt_detail(self, payload: dict[str, Any]) -> str:
        return (
            ...
            f"To: {payload.get('to_address')}"
        )

    def render_ops_placeholder(self) -> str:
        return "[b]No smart-account operation loaded[/b]\nRun a Safe or ERC-4337 action to inspect the payload here."

    def render_data_panel(self, payload: dict[str, Any]) -> str:
    ...
    def render_send_preview(self, preview: dict[str, Any]) -> str:
        ...
        if preview["requires_strong_confirmation"]:
            confirmation = (
                "[b][yellow]Protected send[/yellow][/b]\n"
                f"Type destination suffix: {preview['to_address'][-6:]}\n"
                f"Retype amount: {preview['amount']}\n\n"
            )
        else:
...
def first_journal_hash(rows: list[dict[str, Any]]) -> str | None:
    if not rows:
        return None
    return rows[0]["tx_hash"]
